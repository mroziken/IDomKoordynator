...
def updateLogSerial(msgtype,dev,msg):
	...
	try:
		...
		print sqlstmt
		cur.execute(sqlstmt)
                #cur.execute('''INSERT INTO logserial (TS,MSGTYPE,DEV,MSG) VALUES('?','?','?','?')''',(datetime.now(),msgtype,dev,msg))
                db.commit()
        except  sqlite3.Error, e:
                print "Error: %s" % e.args[0]
                result=False
...
while True:
	...
	if (cmdRows):
		for cmdRow in cmdRows:
			...
			cmd=cmdRow[2]
			if (xbeeSend(cmd)):
				print 'Write to serial successful'
				updResult=updateCmdStatus(ts,dev,cmd,'S')
			else:
